...
export function Companies() {
  ...
  const columns: Column[] = [
    ...
    {
      name: 'Category',
      selector: (row: Company) => row.category
    },
    {
      name: 'Auditee Location',
      selector: (row: Company) => row.address
    },
    {
      name: 'Sector',
      selector: (row: Company) => row.sector
    },
    {
      name: 'HashValue',
      selector: (row: Company) => row.hash_value
    },
  ...
  const handleDelete = async (ids: number | number[]) => {
    if(!currentUserCan('Manage Project')){
      return false;
    }
