...
function FilterItem(props: {item: {label: string, value: string}, index: number, kbIndex: number, name: string, onClick: (value: string) => void}) {
  ...
  return (
    ...
    id={`item-${name}-${index}`} 
    className={`p-2 cursor-pointer dark:bg-black dark:text-white ${kbIndex === index ? 'bg-blue text-white' : 'bg-gray-lightest text-gray-darkest hover:bg-blue hover:text-green-600 font-bold'}`} 
    key={index}
  >
    {display}
  </div>
